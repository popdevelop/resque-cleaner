    ...
    def self.included(base)
      ...
      base.class_eval do
        helpers do
          ...
          def version_filter(id, name, versions, value)
            html = "<select id=\"#{id}\" name=\"#{name}\">"
            html += "<option value=\"\">-</option>"
            versions.each do |version|
              selected = version.to_s == value ? 'selected="selected"' : ''
              html += "<option #{selected} value=\"#{version}\">#{version}</option>"
            end
            html += "</select>"
          end

          def site_filter(id, name, sites, value)
            html = "<select id=\"#{id}\" name=\"#{name}\">"
            html += "<option value=\"\">-</option>"
            sites.each do |site|
              selected = site.to_s == value ? 'selected="selected"' : ''
              html += "<option #{selected} value=\"#{site}\">#{site}</option>"
            end
            html += "</select>"
          end
        ...
        get "/cleaner_list" do
          ...
          @alljobs = cleaner.select
          @klasses = cleaner.stats_by_class(@alljobs).keys
          @exceptions = cleaner.stats_by_exception(@alljobs).keys
          @versions = cleaner.stats_by_version(@alljobs).keys
          @sites = cleaner.stats_by_site(@alljobs).keys
          @errors = cleaner.stats_by_error(@alljobs).keys
          @count = @failed.size
